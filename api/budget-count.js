...
module.exports = async (req, res) => {
  ...
  try {
    const url = `${BASE_URL}/v1/informationbrowser/reports/${REPORT_ID}/count?companyId=${COMPANY_ID}`;
    ...
    if (!unit4Res.ok) return res.status(unit4Res.status).json({ error: 'Unit4 API error', detail: data });
